...
describe('Testing matchMode', () => {
  ...
  test('returns false if there is no match', () => {
    ...
    let testCase = Music.matchMode(new PitchCollection(pitches), new Mode(mode, pitchCenter));
  ...
  test('returns false if there is no match after offset', () => {
    ...
    let testCase = Music.matchMode(new PitchCollection(pitches), new Mode(mode, pitchCenter), scaleDegree);
    expect(testCase).toBe(false);
  })
  ...
  test('returns correct Mode object if there is a match with non-zero pitch center', () => {
    let [pitches, mode, pitchCenter, pitchCenterAdj] = [[0, 2, 3, 6], [0, 3], 1, 0];
    let testCase = Music.matchMode(new PitchCollection(pitches), new Mode(mode, pitchCenter));
    let output = new Mode(pitches, pitchCenter + pitchCenterAdj);
    expect(testCase.getAbstractPitches()).toStrictEqual(output.getAbstractPitches());
    expect(testCase.getAbsolutePitches()).toStrictEqual(output.getAbsolutePitches());
...
describe('Testing matchAll', () => {
  ...
  test('returns an empty array if there are no possible mathces', () => {
    ...
    let testCase = Music.matchAll(new PitchCollection(pitches), new Mode(mode, pitchCenter));
